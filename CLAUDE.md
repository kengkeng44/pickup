...
> 完整版本軌跡(v0.8 → 最新)已搬到獨立檔 **[`CHANGELOG.md`](./CHANGELOG.md)**(v2.0.B.347 拆出,減 CLAUDE.md context 負擔)。
> 接手時**不必通讀**;最新狀態一律跑 `git log --oneline -15`。

**里程碑速覽**(細節去 CHANGELOG):
- **v0.8-0.13** — 8 章 + Ghibli 美學 + 難度系統 + 極簡 splash
- **v1.0-v1.7.5** — rebrand WordWar → Pickup、isometric chibi 主角、4-tab BottomNav
- **v1.7.6-v1.9.x** — 整段 Duolingo parity(題型 / HUD / streak / 成就 / flat icon / 暖色 token)
- **v1.9.50** — grandma-v4「奶奶睡前故事」framework 上線(市場 pivot adult → 兒童童話)
- **v2.0.0** — Duolingo-nested redesign(LessonSchema discriminatedUnion + 單 lesson scope + 每章 24 lessons)
- **v2.0.B.231** — 客群 pivot 下班族 → 兒童/親子;sunset「下班逃逸」字眼
- **v2.0.B.250+** — cron audit 閉環(content/walk/code 每 3hr 自動跑)+ cockpit Decision Board
- **v2.0.B.342-347** — comprehension 合併 + listen-pairs 配對題型 + **介面語言中/英切換 Phase 1**
...
*Last updated: 2026-06-20 — Version History 拆到 CHANGELOG.md(瘦身 ~40% context)。最新狀態見 `git log`。*
