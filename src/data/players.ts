...
const GLOBAL_PREFIXES = [
  'pickup.players',
  'pickup.player.',
  // v2.0.B.464 (per user「玩家二是韓國玩家」): 語言改成「跟著帳號走」(per-profile),
  // 這樣玩家一中文 / 玩家二韓文可以各自獨立。其餘裝置偏好仍全域。
  'pickup.theme',
  'pickup.audio.',
  'pickup.backend.',
  'pickup.analytics',
...
export function ensureProfilesInit(): void {
  ...
  if (existing) { migrateKoTestPlayer(existing); return; }
  const p1: PlayerMeta = { id: genId(), name: '玩家 1', createdAt: 0 };
  const p2: PlayerMeta = { id: genId(), name: '玩家 2（韓國 한국）', createdAt: 0 };
  // p1 = 現有工作集 (不動 LS, 它就是 p1 的 live data)。
  // p2 = 韓國測試帳號: 預設語言韓文 (per user「玩家二是韓國玩家」)。
  try { s.setItem(DATA_PREFIX + p2.id + '.data', JSON.stringify({ 'pickup.lang': 'ko' })); } catch {}
  writeRegistry({ activeId: p1.id, list: [p1, p2] });
}

// 既有使用者 (registry 已存在) 一次性遷移: 把第 2 個帳號設成韓國玩家 (lang ko + 改名),
// 只在它還沒設過語言時動 (不覆蓋使用者後來的選擇)。
function migrateKoTestPlayer(r: Registry): void {
  const s = ls();
  if (!s || r.list.length < 2) return;
  const p2 = r.list[1];
  if (p2.id === r.activeId) return; // 別動正在用的帳號
  try {
    const raw = s.getItem(DATA_PREFIX + p2.id + '.data');
    const data = raw ? JSON.parse(raw) : {};
    if (data['pickup.lang']) return; // 已設過語言 → 不動
    data['pickup.lang'] = 'ko';
    s.setItem(DATA_PREFIX + p2.id + '.data', JSON.stringify(data));
    if (!/한국|韓國|ko/i.test(p2.name)) { p2.name = '玩家 2（韓國 한국）'; writeRegistry(r); }
  } catch {}
}
