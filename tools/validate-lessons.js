...
for (const file of files) {
  ...
  try {
    ...
    const contentIssues = lintContent(raw, file);           // E3 (WARN by default)
    r2Total += r2Issues.length;
    const lintIssues = [...mirrorIssues, ...extendedIssues, ...r2Issues, ...culturalIssues, ...nonWordIssues, ...tfPolarityIssues, ...cultBridgeIssues, ...ngramIssues, ...stimulusIssues, ...pictureNpIssues, ...antonymMirrorIssues, ...blankPosIssues, ...explainVoiceIssues, ...expRegisterIssues];
    const allIssues = [...lintIssues, ...contentIssues];
    // content issues 分開計數：MIRROR_LINT_STRICT 不該因為既有內容違反 E3
    // 就讓 build 掛掉，兩個開關各管各的。
    ...
    if (allIssues.length > 0) {
      ...
      if (STRICT && lintIssues.length > 0) allOk = false;
