...
const lessons = [
  ...
  // ────────────────────────────────────────────────────────────────────
  {
    ...
    questions: [
      ...
      // v2.0.B.228 P1 (Ch5 walkthrough): paraphrase rule violated. Q5 already
      // said 'kept quiet' → option 'staying quiet' = Jaccard mirror. Rewrite
      // option to paraphrase ('holding herself together'), require inference.
      mc('kt-ch5-l1-q6',
        'She held her tears inside and kept her face still.',
        'How did Vasilisa act?',
        ['crying out loud', 'holding herself together', 'running away', 'singing songs'],
        ['大聲哭', '把自己撐住', '跑走', '唱歌'],
        1,
        '推理:壓住眼淚、臉不動 → 心裡撐著 → 把自己撐住。'),
      nar('kt-ch5-l1-q7',
        'One cold night, the fire in the small house went out.',
        '一個冷冷的夜裡,小屋裡的火滅了。'),
      // v2.0.B.228 P1 (Ch5 walkthrough): stem 14 字 + 'flames' 'supper' B1+
      // 違反 canon ≤11 字. Shorten to 3 sparse sentences (8 字 total, NGSL 1000).
      mc('kt-ch5-l1-q8',
        'No fire. The house gets cold. No hot food.',
      ...
      emoji('kt-ch5-l1-q9',
        ...
        2,
        '冷冷的夜 → 冬天。'),
      // v2.0.B.228 P1 (Ch5 walkthrough): old option 直接 spoil Q11 narration
      // ('Go into the deep woods. Get fire from Baba Yaga'). 改 inference about
      // new woman's character (cruel/kind/busy/tired) — 不前瀉劇情, 仍有推理含金量.
      inferLc('kt-ch5-l1-q10',
        'The new woman looked at Vasilisa and smiled with cold eyes.',
        'What kind of person is the new woman?',
        ['kind and warm',
         'cruel under a smile',
         'tired and sad',
         'busy with cooking'],
  ...
  // ────────────────────────────────────────────────────────────────────
  {
    ...
    questions: [
      ...
      nar('kt-ch5-l5-q4',
        ...
        '屋外,一個大大圓圓的東西從樹林過來。'),
      // v2.0.B.228 P1 (Ch5 walkthrough): old sentence 'sat inside ... pushed
      // with stick' 完整答案在內. Rewrite — clue 拉到 Q4 narration ('something
      // big and round came through trees'), Q5 強迫 inference.
      mc('kt-ch5-l5-q5',
        'Outside, the stone bowl came closer through the trees.',
        'How did Baba Yaga move?',
        ['walking', 'riding inside a bowl', 'flying like a bird', 'on a horse'],
        ['用走的', '坐在碗裡', '像鳥一樣飛', '騎馬'],
        1,
        '推理:Q4「big and round」+ Q5「stone bowl came closer」→ 坐在碗裡。'),
      nar('kt-ch5-l5-q6',
