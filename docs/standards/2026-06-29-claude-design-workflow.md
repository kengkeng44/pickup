...
| 步驟 | 在哪做 | 做什麼 |
|------|--------|--------|
| 1️⃣ **Pull 設計系統** | Claude Code 裡跑 `/design-sync` | 把 Pickup 本地的設計系統(`src/data/tokens.ts` 語意色票 + `src/react-app/ui/components/` Button/Card/ProgressBar + renderers 題型)匯入一個 Claude Design 專案。之後 canvas 生的畫面都用**你真正的元件 + tokens**。 |
...
| 選項 | 適合度 | 說明 |
|------|--------|------|
| 🅰️ **App UI(建議先做)** | ⭐⭐⭐⭐ | Pickup 已有 `ui/components/` + `tokens.ts` + 20 種題型 renderer = 現成設計系統種子。`/design-sync` pull 進去後,新畫面(英檢入口、連勝、地圖改版、新題型)先在 canvas 迭代再 handoff。ROI 最高。 |
